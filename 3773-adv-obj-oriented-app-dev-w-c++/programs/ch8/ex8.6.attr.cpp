...
//    typedef map<string, Attr>::const_iterator ITER; //bidirectional

  class AttributedImp : public AttributedIfc {  // implements
  public:
  ...
  void AttributedImp::add(const Attr& a) {
    impl_[a.getName()] = a;
  }
...
// map<string, Attr>::iterator i = impl_.find(name);
  ...
  AttributedImp::ITER AttributedImp::begin() {
    return impl_.begin();
  }

  AttributedImp::ITER AttributedImp::end() {
    return impl_.end();
  }
