...
template <typename T>
Array<T>::Array(size_t size) : size_(size) { array_ = new T[size]; }

template <typename T>
Array<T>::Array(const Array<T>& ar) : size_(ar.size_)
{
  array_ = new T[ar.size_];
  for (size_t i = 0; i < size_; i++) array_[i] = ar.array_[i];
}
...
template <typename T>
T& Array<T>::operator[](int i) { return array_[i]; } // modifier

template <typename T>
const T& Array<T>::operator[](int i) const { return array_[i]; } // query
