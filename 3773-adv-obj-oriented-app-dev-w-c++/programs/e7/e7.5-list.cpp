...
{
  T junk;
  while (size_ > 0) removeFirst(junk);
}
...
List::ListIterator List::begin() { return List::ListIterator(head_, *this); }

List::ListIterator List::end() { return ListIterator(0, *this); }

// prefix
List::ListIterator& List::ListIterator::operator++() throw(domain_error)
{
  if (current_ == 0) throw domain_error("trying to use ++ for empty iterator");   
  current_ = current_->next_;
  return *this;
}

// postfix
List::ListIterator operator++(List::ListIterator& i, int) throw(domain_error)
{
